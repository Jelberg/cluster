...
void main(int argc, char** argv)
{
   ...
   FILE *fichero;
	...
	if (my_id == 0 ){
		   fichero = fopen("/local_home/jelberg.12/CONTADOS.txt", "r+");
...
void Mezcla(FILE *fich)
{
   ...
   do {
      ...
      aux[0] = fopen("/local_home/jelberg.12/aux1.txt", "w+");
      aux[1] = fopen("/local_home/jelberg.12/aux2.txt", "w+");
      rewind(fich);
      Separar(fich, aux);
      rewind(aux[0]);
      rewind(aux[1]);
   ...
   remove("/local_home/jelberg.12/aux1.txt");
   remove("/local_home/jelberg.12/aux2.txt");
}
